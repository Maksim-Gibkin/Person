...
import LabelSkill from "./LabelSkill/LabelSkill";
import InputNumber from "../InputNumber/InputNumber";

interface ICardProps {
  nameObj: { view: string; name: string };
  value: number;
  setAttributes: (atr: Attributes) => void;
  skills: Skill[];
  setSkills: (skl: number) => void;
...
const Card = ({
  ...
  return (
    <div className="card-wrap" onClick={() => setEdit(false)}>
      <div className="card-header">
        <h3 className="card-name">{nameObj.view}</h3>
      ...
      {skills.map((skl: Skill) => (
        <LabelSkill
          key={skl.id}
          skill={skl}
          upLvlSkill={setSkills}
          viewUpBtn={skl.value < 5 && value > skl.value}
        />
      ))}
      <button
        className="card-edit"
